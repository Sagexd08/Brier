//! Brier-score slashing arithmetic.
//!
//! This is a deliberate re-implementation of the EVM `BrierMath.sol` library,
//! not a translation of convenience: the mechanism must be identical on both
//! chains, so the formula, the WAD scale, and the rounding direction all match.
//!
//! What does NOT match is the default safety of the host language. Solidity
//! 0.8 reverts on overflow; **Rust wraps in release mode**. Every arithmetic
//! site below therefore uses `checked_*` and returns an error rather than
//! relying on `overflow-checks` being set (it is set in Cargo.toml as a second
//! line of defence, but a library that is only safe because of a profile flag
//! is not safe).

use anchor_lang::prelude::*;

use crate::errors::BrierError;
use crate::state::{BPS_DENOMINATOR, WAD};
...
pub fn squared_error(confidence: u128, outcome_correct: bool) -> Result<u128> {
    ...
    diff.checked_mul(diff)
        ...
        .checked_div(WAD)
        .ok_or(BrierError::MathOverflow.into())
}
...
pub fn raw_slash(stake: u64, confidence: u128, outcome_correct: bool) -> Result<u64> {
    ...
    u64::try_from(slash).map_err(|_| BrierError::MathOverflow.into())
}
